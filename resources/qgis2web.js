...
map.getView().fit([12275690.522875, -862081.640433, 12278388.005576, -860572.416167], map.getSize());
...
//polygon search by name
var searchHighlightLayer = new ol.layer.Vector({
    source: new ol.source.Vector({
        features: [],
...
var searchDatalist = document.createElement('datalist');
searchDatalist.id = 'polygon-names-list';

// Get all unique names from the layer
function populateNameList() {
    if (!lyr_union_baru_2) {
        return;
    }
    
    var source = lyr_union_baru_2.getSource();
    var nameSet = new Set();
    
    source.forEachFeature(function(feature) {
        var penguasaan = feature.get('Penguasaan');
        if (penguasaan && penguasaan.toString().trim() !== '') {
            nameSet.add(penguasaan.toString().trim());
        }
    });
    
    // Sort names alphabetically
    var sortedNames = Array.from(nameSet).sort();
...
searchInput.className = 'polygon-search-input';
searchInput.placeholder = 'Cari berdasarkan nama...';
searchInput.id = 'polygon-search-input';
searchInput.setAttribute('list', 'polygon-names-list');
searchInput.setAttribute('autocomplete', 'off');

var searchControl = document.createElement('div');
searchControl.className = 'ol-unselectable ol-control polygon-search-control';
searchControl.appendChild(searchInput);
searchControl.appendChild(searchDatalist);

// Populate the list when layer is ready
// Use setTimeout to ensure layer is fully loaded
setTimeout(function() {
    if (lyr_union_baru_2) {
        var source = lyr_union_baru_2.getSource();
        if (source && source.getState() === 'ready') {
            ...
            // Also try to populate immediately in case features are already loaded
...
var searchControlOL = new ol.control.Control({
    element: searchControl
});
map.addControl(searchControlOL);

function searchPolygonsByName(searchName) {
    // Clear previous search results
    searchHighlightLayer.getSource().clear();
    ...
    if (!lyr_union_baru_2) {
        console.log('Layer union_baru_2 tidak ditemukan');
        return;
    }
    
    var searchTerm = searchName.trim().toLowerCase();
    ...
    source.forEachFeature(function(feature) {
        var penguasaan = feature.get('Penguasaan');
        if (penguasaan && penguasaan.toString().toLowerCase().includes(searchTerm)) {
            matchingFeatures.push(feature);
            // Clone the feature for highlighting (transform to view projection if needed)
            var geometry = feature.getGeometry();
            if (geometry) {
                var geomClone = geometry.clone();
                try {
                    var gExtent = geomClone.getExtent();
                    if (gExtent && gExtent[0] >= -180 && gExtent[2] <= 180 && gExtent[1] >= -90 && gExtent[3] <= 90) {
                        // likely EPSG:4326 lon/lat - transform to view projection
                        geomClone.transform('EPSG:4326', map.getView().getProjection());
                    }
                } catch (e) {
                ...
                searchHighlightLayer.getSource().addFeature(clonedFeature);
            }
        }
    });
    
    if (matchingFeatures.length > 0) {
        console.log('searchPolygonsByName: found', matchingFeatures.length, 'matches');
        // Create extent from all matching features (ensuring projection matches view)
        var extent = ol.extent.createEmpty();
        var viewProj = map.getView().getProjection();
        var validExtentAdded = false;
        matchingFeatures.forEach(function(feature, idx) {
            var geometry = feature.getGeometry();
            if (geometry) {
                var geomClone = geometry.clone();
                var origExtent = geomClone.getExtent();
                console.log('searchPolygonsByName: feature', idx, 'origExtent=', origExtent);
                var transformedExtent = origExtent;
                try {
                    if (origExtent && origExtent[0] >= -180 && origExtent[2] <= 180 && origExtent[1] >= -90 && origExtent[3] <= 90) {
                        // extent is lon/lat degrees -> transform
                        transformedExtent = ol.proj.transformExtent(origExtent, 'EPSG:4326', viewProj);
                    }
                } catch (e) {
                    console.warn('searchPolygonsByName: transformExtent failed for feature', idx, e);
                }
                console.log('searchPolygonsByName: feature', idx, 'transformedExtent=', transformedExtent);
                if (transformedExtent && transformedExtent.every(function(n) { return isFinite(n); })) {
                    var w = ol.extent.getWidth(transformedExtent);
                    var h = ol.extent.getHeight(transformedExtent);
                    // Reject extents that are unreasonably large (likely invalid)
                    if (w > 0 && h > 0 && w < 4e7 && h < 4e7) {
                        ol.extent.extend(extent, transformedExtent);
                        validExtentAdded = true;
                    } else {
                        console.warn('searchPolygonsByName: transformedExtent unreasonable size', transformedExtent, 'w,h', w, h);
                    }
                } else {
                    console.warn('searchPolygonsByName: transformedExtent invalid or non-finite', transformedExtent);
                }

                // Add transformed clone to highlight layer for visual feedback (ensure in viewProj)
                try {
                    var g = geomClone.clone();
                    if (origExtent && origExtent[0] >= -180 && origExtent[2] <= 180 && origExtent[1] >= -90 && origExtent[3] <= 90) {
                        g.transform('EPSG:4326', viewProj);
                    }
                    searchHighlightLayer.getSource().addFeature(new ol.Feature({ geometry: g }));
                } catch (e) {
                    console.warn('searchPolygonsByName: could not add highlight feature', e);
                }
            }
        });
        
        // Prefer using the highlight layer extent (it's already transformed). Fallback to computed extent if necessary
        var highlightExtent = searchHighlightLayer.getSource().getExtent();
        console.log('searchPolygonsByName: highlightExtent =', highlightExtent);
        if (highlightExtent && ol.extent.getWidth(highlightExtent) > 0 && ol.extent.getHeight(highlightExtent) > 0 && ol.extent.getWidth(highlightExtent) < 4e7 && ol.extent.getHeight(highlightExtent) < 4e7) {
            console.log('searchPolygonsByName: using highlightExtent to fit');
            map.getView().fit(highlightExtent, {
                padding: [50, 50, 50, 50],
                duration: 1000,
                maxZoom: 18
            });
        } else if (validExtentAdded && extent && ol.extent.getWidth(extent) > 0 && ol.extent.getHeight(extent) > 0) {
            console.log('searchPolygonsByName: computed extent (viewProj) =', extent);
            // Zoom to fit all matching polygons with padding
            ...
            console.warn('searchPolygonsByName: no valid transformed extents; falling back to centering target coords and zoom 18');
            // Fallback: center on target coordinates and set zoom 18
            try {
                var targetLonLat = [110.28678073217094, -7.7189936628769615];
                var targetCenter = ol.proj.transform(targetLonLat, 'EPSG:4326', viewProj);
                map.getView().animate({ center: targetCenter, zoom: 18, duration: 1000 });
            } catch (err) {
                try {
                    var fallbackCenter = ol.proj.fromLonLat([110.28678073217094, -7.7189936628769615]);
                    map.getView().animate({ center: fallbackCenter, zoom: 18, duration: 1000 });
                } catch (e) {
                    console.warn('searchPolygonsByName: Could not set fallback center:', e);
                }
            }
        }
    } else {
        // No results found - could show a message
        console.log('Tidak ada polygon dengan penguasaan: ' + searchName);
        alert('Tidak ada polygon dengan nama: ' + searchName);
    }
}
...
searchInput.addEventListener('keypress', function(e) {
    if (e.key === 'Enter') {
        ...
        searchPolygonsByName(searchValue);
    }
});

// Search when user selects from dropdown (change event fires on selection)
...
//move controls inside containers, in order
    ...
    if (typeof searchControl !== 'undefined') {
        topLeftContainerDiv.appendChild(searchControl);
    }
    //scale line
